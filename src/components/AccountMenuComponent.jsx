...
export const AccountMenuComponent = ({ onSignout, children }) => {
  ...
  return (
    <Menu
      menuButton={
        <StyledMenuButton>{children}</StyledMenuButton>
      }
      key='bottom'
      direction='bottom'
      align='end'
    ...
    >
      {
        profiles.map((profile) => (
          <MenuItem key={profile.id} disabled={userprofile.profileName === profile.name ? true : false} onClick={(event) => handleChangeProfile(event, profile)}>
            {
              profile.isKids === true ? <Image alt='profile' src='https://www.kiyotaro.cloud/images/tealium_beast.png' width='40px' /> 
                                          : <Image alt='profile' src='https://www.kiyotaro.cloud/images/tealium-logo-small.jpeg' width='40px' /> 
            }
            {profile.name}
          </MenuItem>
        ))
